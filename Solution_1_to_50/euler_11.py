...
<h2>Largest product in a grid</h2><div id="problem_info" class="info"><h3>Problem 11</h3><span>Published on Friday, 22nd February 2002, 06:00 pm; Solved by 136026; Difficulty rating: 5%</span></div>
...
def brute_force():
  ...
  for i in range(0, 20):
    for j in range(0, 20-3):
      p = xy[i][j]* xy[i][j+1]* xy[i][j+2]* xy[i][j+3]
      m_max = max(m_max, p)

  # up/down
  for j in range(0, 20):
    for i in range(0, 20-3):
      p = xy[i][j]* xy[i+1][j]* xy[i+2][j]* xy[i+3][j]
      m_max = max(m_max, p)   

  # right diagonally
  for i in range(0, 20-3):
    for j in range(0, 20-3):
      p = xy[i][j]* xy[i+1][j+1]* xy[i+2][j+2]* xy[i+3][j+3]
      m_max = max(m_max, p)

  # left diagonally
  for i in range(0, 20-3):
    for j in range(19, 0+2, -1):
      #print xy[i][j], xy[i+1][j-1], xy[i+2][j-2], xy[i+3][j-3]
      p = xy[i][j]* xy[i+1][j-1]* xy[i+2][j-2]* xy[i+3][j-3]
      m_max = max(m_max, p)
  print "max=", m_max    
