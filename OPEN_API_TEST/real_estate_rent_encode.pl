...
my $TELEGRAM_TOKEN = '*****6775:AAHzhy6ZE2BoTrBbw49PVhe-Pqwnm9M16Es';

my $RENT_KEY = '*****t4sbGXo39M2uKPj%2B1zmjDPiJEzt%2BkLXpoLiKMTua6fsl5PWikGlgIuNthm1WXm2WVtMTdwSoIjuH5fR0g%3D%3D';
...
sub get_apartment_rent {
    ...
    for (my $i=0; $i < scalar @fee_list; $i++) {
        ...
        if ($fee == 0) {
            $result .= sprintf ("전세   :%s, %s %s(%s층), %sm² (%s/%s)\n", 
				$deposit, $district, $apt, $floor, $size, $month, $day);
		}
        else {
            $result .= sprintf ("반전세 :%s(%s), %s %s(%s층), %sm² (%s/%s)\n", 
...
sub help() {
    return "사용법: \n\
1.  부동산 전세 조회 : /rent 11410 201512 [대흥동] \
2.  태영아파트 조회 : /ty [면적] [연도] \
